...
;(function(window) {
  ...
  function runInContext(context) {
     ...
     * // => { 'name': 'moe' }
     */
    var pick = function pick(callback, object, thisArg) {
      var result = {};
      if (typeof callback != 'function') {
        var index = -1,
     ...
     * // => { '3': 2, '5': 1 }
     */
    var countBy = function countBy(callback, collection, thisArg) {
      var result = {};
      callback = lodash.createCallback(callback, thisArg);

      forEach(function(value, key, collection) {
     ...
     * // => false
     */
    var every = function every(callback, collection, thisArg) {
      var result = true;
      callback = lodash.createCallback(callback, thisArg);
     ...
     * // => [{ 'name': 'apple', 'organic': false, 'type': 'fruit' }]
     */
    var filter = function filter(callback, collection, thisArg) {
      var result = [];
      callback = lodash.createCallback(callback, thisArg);
     ...
     * // => { 'name': 'banana', 'organic': true, 'type': 'fruit' }
     */
    var find = function find(callback, collection, thisArg) {
      callback = lodash.createCallback(callback, thisArg);
     ...
     */
    var forEach = function forEach(callback, collection, thisArg) {
      var index = -1,
          length = collection ? collection.length : 0;
     ...
     * // => { '3': ['one', 'two'], '5': ['three'] }
     */
    var groupBy = function groupBy(callback, collection, thisArg) {
      var result = {};
      callback = lodash.createCallback(callback, thisArg);
     ...
     * // => ['moe', 'larry']
     */
    var map = function map(callback, collection, thisArg) {
      var index = -1,
          length = collection ? collection.length : 0;
     ...
     * // => { 'name': 'larry', 'age': 50 };
     */
    var max = function max(callback, collection, thisArg) {
     ...
     * // => { 'name': 'moe', 'age': 40 };
     */
    var min = function min(callback, collection, thisArg) {
      var computed = Infinity,
          result = computed;
     ...
     * // => ['moe', 'larry']
     */
    var pluck = function pluck(property, collection) {
      var index = -1,
          length = collection ? collection.length : 0;
      ...
      return result || map(property, collection);
    }.autoCurry();
     ...
     * // => { 'a': 3, 'b': 6, 'c': 9 }
     */
    var reduce = function reduce(callback, accumulator, collection, thisArg) {
     ...
     * // => [4, 5, 2, 3, 0, 1]
     */
    var reduceRight = function reduceRight(callback, accumulator, collection, thisArg) {
     ...
     * // => [{ 'name': 'carrot', 'organic': true, 'type': 'vegetable' }]
     */
    var reject = function reject(callback, collection, thisArg) {
      callback = lodash.createCallback(callback, thisArg);
      return filter(function(value, index, collection) {
     ...
     * // => false
     */
    var some = function some(callback, collection, thisArg) {
      var result;
      callback = lodash.createCallback(callback, thisArg);
     ...
     */
    var sortBy = function sortBy(callback, collection, thisArg) {
      var index = -1,
          length = collection ? collection.length : 0,
          result = Array(typeof length == 'number' ? length : 0);
     ...
     * // => 1
     */
    var findIndex = function findIndex(callback, array, thisArg) {
      var index = -1,
          length = array ? array.length : 0;
     ...
     * // => 2
     */
    var indexOf = function indexOf(value, array, fromIndex) {
     ...
     * // => 1
     */
    var lastIndexOf = function lastIndexOf(value, array, fromIndex) {
  ...
  var drop = function(n, array) {
    ...
    // var rest = function rest(array, callback, thisArg) {
    //   if (typeof callback != 'number' && callback != null) {
    //     var n = 0,
    //         index = -1,
    //         length = array ? array.length : 0;
	    ...
	    //+ exposeFunction :: f -> Bool
	  	...
	  	//+ expose :: a -> IO
		    , expose = function(ns) {
		        ...
		        ns = ns || window;
		        for (f in lodash) {
		          if (exposeFunction(f) && lodash.hasOwnProperty(f)) {
		            ns[f] = lodash[f];
		          }
		        }
		      }
